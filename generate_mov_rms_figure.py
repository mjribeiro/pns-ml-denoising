...
ch1_bp_recording = ch1_bp_recording[:max_idx]
...
spl = make_interp_spline(peaks, ch1_bp_recording[peaks])
...
xnew = np.linspace(min(peaks), max(peaks), len(ch1_bp_recording))
xnew_sec = np.linspace(min(peaks_x), max(peaks_x), len(ch1_bp_recording))

spl_y = spl(xnew)

# Lowpass filter envelope so it's smoother
# bp_envelope = butter_lowpass_filter(spl_y, 15, 1000)
bp_envelope = butter_lowpass_filter(spl_y, 100, fs)
# bp_envelope = spl_y

ax4.plot(time_full, ch1_bp_recording)
# ax4.plot(xnew, bp_envelope)
ax4.plot(xnew_sec, bp_envelope)
...
for ch in range(num_chs):
    ...
    if ch == 0:
        # Taking moving RMS windows later on will produce NaNs at start of array, so crop
        # BP recording accordingly
        # Only needs doing once, but needs size of "bandpass_rms_norm"
        ignore_indices = bp_envelope_norm.shape[0] - bandpass_rms_norm.shape[0]

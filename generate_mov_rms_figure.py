...
# Load n2n results
n2n_noisy_inputs_ch1 = np.load("./results/n2n_noisy_input_ch1.npy")
...
zoom_n2n_noisy_inputs = n2n_noisy_inputs_ch1[zoom_start:zoom_end]
...
orig_filtered_win = bandpass_filter(zoom_n2n_noisy_inputs, [250, 10e3], 100e3)
orig_filtered = bandpass_filter(n2n_noisy_inputs_ch1, [250, 10e3], 100e3)
...
bandpass_rms = scaler.fit_transform(bandpass_rms.dropna().to_numpy().reshape(-1, 1))
cvae_reconstr_ch1_rms = scaler.fit_transform(cvae_reconstr_ch1_rms.dropna().to_numpy().reshape(-1, 1))
n2n_reconstr_ch1_rms = scaler.fit_transform(n2n_reconstr_ch1_rms.dropna().to_numpy().reshape(-1, 1))

bandpass_x_corr = resample(bandpass_rms, len(bp_envelope_norm)).ravel()
cvae_x_corr = resample(cvae_reconstr_ch1_rms, len(bp_envelope_norm)).ravel()
n2n_x_corr = resample(n2n_reconstr_ch1_rms, len(bp_envelope_norm)).ravel()
